...
const NewsCard = ({ id, title, author, publishedAt, content, likeCount: initialLikeCount, tags = [] }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 px-6 sm:px-10 py-6 m-4 rounded-xl shadow-sm max-w-5xl w-full mx-auto transition-colors">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <div className="flex items-center mb-4">
                        {author ? (
                            <Link to={`/profile/${author.id}`} className="flex items-center group">
                                <img src={author.avatar} alt={author.name} className="w-6 h-6 rounded-full mr-2 object-cover"/>
                                <span className="text-sm text-gray-600 dark:text-gray-400 group-hover:underline">{author.name}</span>
                            </Link>
                        ) : (
            ...
            <div
                ref={contentRef}
                className={`relative transition-all duration-300 prose prose-sm max-w-none leading-relaxed text-gray-700 dark:text-gray-300 [&>p]:mb-4 ${isExpanded ? "max-h-none" : "max-h-60 overflow-hidden"}`}
            >
